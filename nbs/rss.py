...
# %% auto #0
__all__ = ["RSS_DATE_FORMAT", "Podcast", "extraire_dureesummary", "extraire_urls_rss"]

# %% py rss helper.ipynb #b661f963
import feedparser
import re
...
def extraire_urls_rss(duree_mini_minutes: int = 15) -> list[str]:
    ...
    Supporte les formats audio/mpeg (MP3) et audio/x-m4a (M4A).
    ...
    for entree in flux.entries:
        for link in entree.links:
            if (
                link.type in audio_types
                and extraire_dureesummary(entree.summary) > duree_mini_minutes * 60
            ):
                urls.append(link.href)
    return urls


# %% py rss helper.ipynb #3e3ea032
...
class Podcast:
    ...
    def get_most_recent_episode_from_DB(self) -> datetime | None:
        """
        ...
        for doc in most_recent_document:
            most_recent_date = doc["date"].replace(tzinfo=UTC)
    ...
    def list_last_large_episodes(
        ...
        for entry in self.parsed_flow.entries:
            ...
            if (
                date_db
                and date_rss > date_db
                and RSS_episode.get_duree_in_seconds(entry.itunes_duration)
