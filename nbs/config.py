...
# %% py config.ipynb #435afd0e
def get_pgx_config() -> dict:
    """Récupère la configuration de la station PGX (transcription automatisée par SSH).

    Toutes les valeurs proviennent des variables d'environnement (voir .env.example) :
    PGX_HOST, PGX_USER, PGX_SSH_KEY_PATH, PGX_REMOTE_AUDIO_ROOT, PGX_REMOTE_TRANSCRIPTION_ROOT.

    Returns:
        dict: Un dictionnaire avec les clés host, user, key_path,
        remote_audio_root, remote_transcription_root.
    """
    load_env()
    return {
        "host": os.getenv("PGX_HOST"),
        "user": os.getenv("PGX_USER"),
        "key_path": os.getenv("PGX_SSH_KEY_PATH"),
        "remote_audio_root": os.getenv("PGX_REMOTE_AUDIO_ROOT"),
        "remote_transcription_root": os.getenv("PGX_REMOTE_TRANSCRIPTION_ROOT"),
    }


# %% py config.ipynb #0bd92633
...
# %% py config.ipynb #3c85edc9


def get_DB_VARS() -> tuple[str | None, str | None, str | None]:
    ...
    DB_LOGS: str | None = os.getenv("DB_LOGS")
    return DB_HOST, DB_NAME, DB_LOGS


# %% py config.ipynb #ef9faa4c
